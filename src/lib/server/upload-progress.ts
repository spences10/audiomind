...
const progress_listeners = new Map<string, Set<(data: ProgressUpdate) => void>>();

// Store for queued updates until client connects
const queued_updates = new Map<string, ProgressUpdate[]>();

export function add_progress_listener(upload_id: string, listener: (data: ProgressUpdate) => void) {
...
export function update_progress(upload_id: string, data: ProgressUpdate) {
    const listeners = progress_listeners.get(upload_id);
