...
const create_upload_state = () => {
	...
	return {
		...
		get start_time() { return state.start_time; },
		get current_operation() { return state.current_operation; },
		...
		update_progress: (data: Partial<ProgressState>) => {
			...
			if (data.current_operation) state.current_operation = data.current_operation;
			
			// Calculate progress if we have current and total
			if (data.current !== undefined && data.total !== undefined) {
				state.progress = Math.min(100, Math.max(0, (data.current / data.total) * 100));
